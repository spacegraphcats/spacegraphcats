...
def main():
    ...
    siglist = [ (sig.name(), sig) for sig in siglist ]

    for name, query_sig in siglist:
        print('---')
        print('searching for MinHash of sequence {}'.format(query_sig.name()))
        ...
        print('Aggregated results match: {:.3f}'.format(mh.compare(query_mh)))
        print('Query match to aggregated results: {:.3f}'.format(query_mh.compare(mh)))
